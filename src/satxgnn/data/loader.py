...
def load_fda_drugs(filepath: Optional[Path] = None) -> pd.DataFrame:
    ...
    Args:
        filepath: JSON 檔案路徑，預設為 data/raw/sa_fda_drugs.json

    Returns:
        包含所有藥品的 DataFrame

    Raises:
        FileNotFoundError: 找不到 SFDA 資料檔案時，提供下載指引
    """
    if filepath is None:
        # loader.py -> data -> satxgnn -> src -> SATxGNN
        filepath = Path(__file__).parent.parent.parent.parent / "data" / "raw" / "sa_fda_drugs.json"

    if not filepath.exists():
        raise FileNotFoundError(
            f"找不到 SFDA 藥品資料: {filepath}\n"
            f"請先執行以下步驟：\n"
            f"1. 執行: uv run python scripts/process_fda_data.py\n"
            f"   (腳本會從 SFDA 下載 Excel 資料)"
        )
