...
安全性相關欄位（`key_warnings`、`contraindications`、DDI 查詢）均為資料缺口或查無結果，其中「SFDA package insert warnings/contraindications」已被標記為 **Blocking** 等級缺口，直接阻擋進入 S1 安全性初評階段。
...
**理由：**
本候選適應症（原發性血小板釋放障礙）僅有 TxGNN 電腦運算分數支持，無任何臨床試驗或文獻佐證（L5），且模型自身機轉分析已指出生物學關聯性薄弱；同時安全性資料存在 Blocking 等級缺口（SFDA package insert warnings/contraindications未取得），藥物也尚未於沙烏地阿拉伯上市，現階段不具備推進條件。
